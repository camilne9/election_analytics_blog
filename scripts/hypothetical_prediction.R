...
hypothetical_prediction <- current_polls %>% 
...
plot_usmap(data = hypothetical_prediction, regions = "states", values = "trump_predicted_winner") + 
...
uncertainty_prediction <- hypothetical_prediction %>% 
...
hypothetical_prediction %>% 
  mutate(State = state, 
         ...
         "Biden Vote Share" = round(100 - prediction, 1)) %>%
  select(State, "Trump Vote Share", "Biden Vote Share") %>% 
  gt() %>% 
...
vep_total %>% 
  ...
  select(state, VEP) %>% 
  right_join(hypothetical_prediction, by = 'state') %>% 
...
win_rate_2020 <- hypothetical_prediction %>% 
